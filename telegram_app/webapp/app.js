...
$("refresh_seconds").addEventListener("input", () => { refreshDirty = true; });
...
async function refresh() { try { const s = await request('/api/state'); if (!thresholdDirty && document.activeElement !== $('threshold')) $('threshold').value = s.threshold; if (!refreshDirty && document.activeElement !== $('refresh_seconds')) $('refresh_seconds').value = s.refresh_seconds; $('status').textContent = s.status; $('substatus').textContent = s.connected ? (s.running ? 'Моніторинг увімкнений' : 'Моніторинг зупинений') : 'Локальний агент не підключений'; $('dot').className = `dot ${s.connected ? 'ok' : ''}`; $('pair').hidden = s.connected; $('pair').disabled = s.paired && s.connected; $('pair').textContent = 'Підключити цей ПК'; $('disconnect').hidden = !s.paired; } catch(e) { $('status').textContent=e.message; } }
async function command(action) { try { const refreshSeconds=Number($('refresh_seconds').value); if (!Number.isFinite(refreshSeconds) || refreshSeconds < 1) throw new Error('Інтервал має бути не менше 1 секунди.'); await request('/api/command',{method:'POST',body:JSON.stringify({action,threshold:Number($('threshold').value),refresh_seconds:refreshSeconds})}); thresholdDirty = false; refreshDirty = false; await refresh(); } catch(e) { tg?.showAlert?.(e.message) || alert(e.message); } }
